...
def is_problematic_crop(img_cropped, xs, ys, roi_size, image_size, problem_threshold):
    ...
    is_problematic = wrong_shape or too_close_to_edge
    if is_problematic:
        print(f"Problematic crop detected: shape={img_cropped.shape}, xs={xs}, ys={ys}. wrong_shape?: {wrong_shape}, too_close_to_edge? {too_close_to_edge}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_ext', type=str, required=False, default='png')
    parser.add_argument('--output_dir', type=str, required=True)
    parser.add_argument('--roi_size', type=int, default=968)
    parser.add_argument('--manual_crop', type=bool, default=True, help="If True, will skip automatic ROI detection and crop the center of the image directly.")
    parser.add_argument('--problem_threshold', type=float, default=0.15)

    args = parser.parse_args()
    roi_size = args.roi_size
    args.input_ext = args.input_ext.lstrip('.').lower()  # Normalize input extension
    data_files = natsorted(glob.glob(f'{args.input_dir}/**/*.{args.input_ext}', recursive=True))
    ...
    for path in tqdm(data_files):
        ...
        name = path.split("/")[-1].replace(f".{args.input_ext.upper()}", f".{args.input_ext}")
        ...
        try:
            if args.input_ext.lower() == 'tif' or args.input_ext.lower() == 'tiff':
                img = tifffile.imread(path)
                # Get image dimensions dynamically
                image_height, image_width, channels = img.shape
                img = img[:, :, 0].astype(np.float32)
                # img = img_as_float(img[:, :, 0])
            else:
        ...
        roi_size = min(roi_size, image_size - 1)  # Ensure ROI size is smaller than image size to avoid out-of-bounds cropping
        print(f"Processing {os.path.basename(path)}: image shape={img.shape}, image_height={image_height}, image_width={image_width}, image_size={image_size}, roi_size={roi_size}")
        
        if args.manual_crop:
            xs = (image_width - roi_size) / 2
            ys = (image_height - roi_size) / 2
            ys = int(max(0, ys))
            xs = int(max(0, xs))
            print(f"\t - Manual crop: xs={xs} : {xs+roi_size}, y={ys} : {ys+roi_size}")
            img_cropped = img[ys:ys+roi_size, xs:xs+roi_size].astype(np.uint8)
        else:
        ...
        Image.fromarray(img_cropped_final).save(save_path+".png", format='PNG')
